...
Task("build")
	...
	.Does(() => DotNetBuild("FreeMobile.slnx", new() { Configuration = release ? "Release" : "Debug" }));
...
Task("publish")
	...
	.Does(() => DotNetPack("FreeMobile.slnx", new() { OutputDirectory = "var" }))
	.Does(() => DotNetNuGetPush($"var/Belin.FreeMobile.{version}.nupkg", new() { ApiKey = EnvironmentVariable("NUGET_API_KEY"), Source = "https://api.nuget.org/v3/index.json" }))
	.Does(() => DotNetNuGetPush($"var/Belin.FreeMobile.{version}.nupkg", new() { ApiKey = EnvironmentVariable("GITHUB_TOKEN"), Source = "https://nuget.pkg.github.com/cedx/index.json" }))
	.DoesForEach(["tag", "push origin"], action => StartProcess("git", $"{action} v{version}"));

Task("test")
	.Description("Runs the test suite.")
	.Does(() => DotNetTest("FreeMobile.slnx", new() { Settings = ".runsettings" }));
